...
class RecipeIngredientUnit
{
	...
	function insert($dbh){
				$sql = "INSERT INTO `recipe_ingredients_units` (`recipe_id`, `ingredient_id`, `unit_id`, `amount`) VALUES (?, ?, ?, ?)";
				
				$stmt = $dbh->prepare($sql);
				$stmt->bindParam(1,$this->recipe_id);
				$stmt->bindParam(2,$this->ingredient_id);
				$stmt->bindParam(3,$this->unit_id);
	...
	static function findAllForRecipe( $recipe_id, $dbh ) {
		$sql = "
				SELECT riu.*, i.name as iname, u.name as uname
